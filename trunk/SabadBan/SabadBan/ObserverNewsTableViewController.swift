...
class ObserverNewsTableViewController: BaseTableViewController {
    ...
    var newsModel = [ObserverNewsModel]()
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.lblNewsDate.attributedText = newsModel[indexPath.row].date
        ...
        Request.postData(url, body: body) {
            ...
            if ((news?.successful) != nil) {
                for i in 0 ..< news!.response.newsDetailsList.count {
                    self.newsModel.append(ObserverNewsModel(
                        title: news!.response.newsDetailsList[i].newsTitle,
                        details: news!.response.newsDetailsList[i].newsReport,
                        date: news!.response.newsDetailsList[i].convertTime(UIColor.orangeColor()),
                        link: ""))
                }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if (segue.identifier == UIConstants.NewsDetailsSeguei) {
            ...
            svc.newsDateAttributed = newsModel[index.row].date
